  ...
  <template>
    {{#if this.canRender}}
      <section class="csl-panel csl-owner-panel" aria-labelledby="csl-owner-panel-title">
        <header class="csl-owner-panel__header">
          <div>
          ...
          <button
            class="csl-button csl-owner-panel__toggle"
            type="button"
            aria-expanded={{this.isOpen}}
            {{on "click" this.toggleOpen}}
          >
            {{if this.isOpen (i18n "crimson_server_list.owner_panel.close") (i18n "crimson_server_list.owner_panel.open")}}
          </button>
        </header>

        {{#if this.isOpen}}
          {{#if this.errorMessage}}
            <div class="csl-owner-panel__error">
          ...
          {{else if this.isLoading}}
            <p class="csl-empty csl-empty--compact" role="status">{{i18n "crimson_server_list.owner_panel.loading"}}</p>
          {{else}}
            {{#if this.stats}}
            ...
            <div class="csl-owner-panel__summary">{{this.showingLabel}}</div>
            ...
            {{#if this.servers.length}}
                ...
                {{#each this.servers as |server|}}
                  <article class="csl-owner-card csl-game--{{server.game_slug}}">
                    ...
                    <div class="csl-owner-card__actions">
                      <a class="csl-button" href={{server.detail_url}}>{{i18n "crimson_server_list.owner_panel.manage"}}</a>
                    </div>
                  </article>
                {{/each}}
              </div>
              ...
              {{/if}}
            {{else}}
              <p class="csl-empty csl-empty--compact">{{i18n "crimson_server_list.owner_panel.empty"}}</p>
            {{/if}}
          {{/if}}
        {{/if}}
      </section>
